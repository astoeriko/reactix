---
title: Conservative transport and first-order decay
jupyter: python3
execute: 
  enabled: true
---
...
Note that the discharge will be saved in the `TransportSystem` object (not in `Advection`) because it needs to be accessed by both `Advection` and `Dispersion` (in order to compute the hydrodynamic dispersivity).
...
The next step is to create a `TransportSystem` object that contains all the previous settings.
